...
import { SelectorRecord, SelectorStore } from './selectorStore';
...
describe('testing selector store part', () => {
  test('createSelectorRecord works correctly', () => {
    ...
    const stateImage = {
      registerSelectorTrigger: jest.fn((selectorTrigger: () => boolean) => {
        memSelectorTrigger = selectorTrigger;

        return readState;
      }),
    } as unknown as StateImage<{}>;

    const selectorRecord = new SelectorRecord(selector, stateImage);
    expect(selector.mock.calls).toHaveLength(0);
    expect(readState.mock.calls).toHaveLength(0);

    expect(selectorRecord.getValue()).toBe(11);
    ...
    const unsubscribeJob1 = selectorRecord.addSubscription(subscriptionJob1);
    expect(selectorRecord.getValue()).toBe(33);
    ...
    expect(selectorRecord.getValue()).toBe(25);
    expect(subscriptionJob1.mock.calls).toHaveLength(1);
    expect(selector.mock.calls).toHaveLength(3);
    expect(readState.mock.calls).toHaveLength(3);
    ...
    const unsubscribeJob2 = selectorRecord.addSubscription(subscriptionJob2);
    expect(selectorRecord.getValue()).toBe(25);
    expect(subscriptionJob1.mock.calls).toHaveLength(1);
    ...
    expect(selectorRecord.getValue()).toBe(333);
    expect(subscriptionJob1.mock.calls).toHaveLength(2);
    expect(subscriptionJob2.mock.calls).toHaveLength(2);
    ...
    expect(selectorRecord.getValue()).toBe(66);
    expect(subscriptionJob1.mock.calls).toHaveLength(2);
    expect(subscriptionJob2.mock.calls).toHaveLength(2);
  ...
  test('createSelectorStore works correctly', () => {
    ...
    const selectorStore = new SelectorStore(stateImage);

    let selectorReturn1 = 199;
    const selector1 = jest.fn(() => selectorReturn1);

    expect(selectorStore.getSelectorValue(selector1)).toBe(199);
    ...
    expect(selectorStore.getSelectorValue(selector1)).toBe(44);
    expect(selector1.mock.calls).toHaveLength(1);
    ...
    selector1.mock.calls = [];
    const unsubscribe1 = selectorStore.subscribeToStateInSelectorStore(selector1, subscription1);
    expect(selector1.mock.calls).toHaveLength(0);
    ...
    expect(selectorStore.getSelectorValue(selector1)).toBe(11);
    ...
    const unsubscribe2_1 = selectorStore.subscribeToStateInSelectorStore(
      selector1,
      subscription2Init
    );
    selectorReturn1 = 77;
    ...
    const unsubscribe2_2 = selectorStore.subscribeToStateInSelectorStore(
      selector2,
      subscription2Init
    );
    expect(memSelectorTrigger).toBeDefined();
    ...
    subscription2Init.mock.calls = [];
    subscription2Final.mock.calls = [];
    expect(selectorStore.getSelectorValue(selector1)).toBe(755);
    expect(selectorStore.getSelectorValue(selector2)).toBe(800);
    expect(selector1.mock.calls).toHaveLength(0);
    ...
    subscription2Init.mock.calls = [];
    subscription2Final.mock.calls = [];
    expect(selectorStore.getSelectorValue(selector1)).toBe(567);
    expect(selector1.mock.calls).toHaveLength(1);
    expect(selector2.mock.calls).toHaveLength(0);
    expect(subscription1.mock.calls).toHaveLength(0);
    expect(subscription2Init.mock.calls).toHaveLength(0);
    ...
    subscription2Init.mock.calls = [];
    subscription2Final.mock.calls = [];
    expect(selectorStore.getSelectorValue(selector1)).toBe(100);
    expect(selectorStore.getSelectorValue(selector2)).toBe(66);
    expect(selector1.mock.calls).toHaveLength(1);
    expect(selector2.mock.calls).toHaveLength(1);
    expect(subscription1.mock.calls).toHaveLength(0);
    expect(subscription2Init.mock.calls).toHaveLength(0);
