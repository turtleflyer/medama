...
export const medamaGuarantiesTest = (createMedama: CreateMedama) => {
  const symbKey = Symbol('symbKey');

  describe('medama pupil guaranties testcases', () => {
  ...
  describe.each([
  ...
  ])('medama pupil guaranties testcases (%s)', (_name, createMedama) => {
    ...
    test('handling errors when subscribing does not leave bad state', () => {
      const { subscribeToState, setState } = createMedama({ a: 1 });
      ...
      expect(() => {
        subscribeToState(({ a }) => a, subscription);
      }).toThrow();
      expect(subscription).toHaveBeenCalledTimes(1);
